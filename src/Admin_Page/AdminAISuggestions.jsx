...
const AdminAISuggestions = () => {
  ...
  const medium = rows.filter(
    (r) => r.expense_ratio >= 50 && r.expense_ratio < 80,
  ).length;
  const good = rows.filter((r) => r.expense_ratio < 50).length;

  return (
    <>  
      <div classtxName="space-y-5">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">
            AI Suggestions Monitor
          </h2>
          <p className="text-sm text-gray-500 mt-1">
            Financial health overview for all users
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {rows.map((row) => {
              ...
              return (
                <div
                  key={row.user_id}
                  className="rounded-2xl p-4 border border-green-200 bg-green-50 shadow-sm hover:shadow-md transition"
                >
      ...
      {selectedUser && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-[95%] max-w-lg shadow-lg">
            ...
            <div className="mt-4">
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                <span>Expense Ratio</span>
                <span>{selectedUser.expense_ratio}%</span>
              </div>

              <div className="w-full bg-gray-100 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${getRatioColor(selectedUser.expense_ratio).bar}`}
                  style={{
                    width: `${Math.min(selectedUser.expense_ratio, 100)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mt-5">
              <div className="bg-gray-50 rounded-xl p-3 text-center">
                <p className="text-xs text-gray-400">Income</p>
                <p className="text-sm font-bold text-green-600">
                  ₹{Number(selectedUser.income).toLocaleString()}
                </p>
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-3 text-center">
                <p className="text-xs text-gray-400">Expense</p>
                <p className="text-sm font-bold text-red-600">
                  ₹{Number(selectedUser.expense).toLocaleString()}
                </p>
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-3 text-center">
                ...
                <p className="text-sm font-bold text-indigo-600">
                  ₹
                  {Number(
                    selectedUser.income - selectedUser.expense,
                  ).toLocaleString()}
                </p>
              </div>
            </div>
